...
On the original and current PCB revisions (26a902**a**/**b** marked on label), the 3.5mm jack is not electrically isolated. If planning to tether the camera to a computer while controlling the shutter using a light source with this PCB, test that the camera and cable are wired correctly first. Connect both the shutter control cable and a USB cable to the camera, and touch the tip of the 3.5mm plug to the exposed metal shell of the USB connector. If this *does not* trigger the camera shutter, the internal wiring of the camera or cable is likely reversed from the de facto standard and connecting the camera to both the light source and computer at the same time may cause issues. This PCB also does not connect the ring (focus) terminal on the 3.5mm jack, and requires a hardware modification to connect this terminal for remote shutter release compatibility with Nikon DSLRs and mirrorless cameras. Units with this modification can be identified by light gray paint applied to the externally visible part of the 3.5mm jack. There is a workaround for using unmodified units with Nikon cameras, which is to connect the camera cable to the light through a 3.5mm mono-to-stereo adapter. If doing this, it is important that the male end of the adapter used is a two-terminal (mono) plug, rather than a three-terminal (stereo) plug.
