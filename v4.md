...
As of the mid-July 2026 PCB revision (26a901**b** marked on label), the shutter and focus signals to the camera are independently controllable from the web app. When the **Send focus signal** checkbox is selected, the focus signal will be held (equivalent to half-pressing the shutter button) for the entire duration that the LEDs are on. A few cameras may need the focus signal in order to trigger the shutter, and on a few other cameras, the focus signal may cause the camera to autofocus even when it is set to manual focus mode. The 3.5mm jack is electrically isolated from the USB ports in this revision.
