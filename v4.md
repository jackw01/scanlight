...
The light source is powered via the **right USB-C port**. A USB power source that can supply at least 2A at 5V (e.g. almost any USB charger from the last 10 years) is required.
...
As of the mid-July 2026 PCB revision (26a901**b** marked on label), the shutter and focus signals to the camera are independently controllable from the web app. When the **Send focus signal** checkbox is selected, the focus signal will be held (equivalent to half-pressing the shutter button) for the entire duration that the LEDs are on. Some cameras (mainly Nikon) may need the focus signal in order to trigger the shutter, and on a few other cameras, the focus signal may cause the camera to autofocus even when it is set to manual focus mode. The 3.5mm jack is electrically isolated from the USB ports in this revision. There is a workaround for using older units with Nikon cameras, which is to connect the camera cable to the light through a 3.5mm mono-to-stereo adapter. If doing this, it is important that the male end of the adapter used is a two-terminal (mono) plug, rather than a three-terminal (stereo) plug.

On the original PCB revision (26a901**a** marked on label), the 3.5mm jack is not electrically isolated. If planning to tether the camera to a computer while controlling the shutter using a light source with this PCB, test that the camera and cable are wired correctly first. Connect both the shutter control cable and a USB cable to the camera, and touch the tip of the 3.5mm plug to the exposed metal shell of the USB connector. If this *does not* trigger the camera shutter, the internal wiring of the camera or cable is likely reversed from the de facto standard and connecting the camera to both the light source and computer at the same time may cause issues.
...
<small>All dimensions in millimeters. This adapter is NOT compatible with the old 35mm toneCarrier design found on Printables.com. As of 2026-05-29, the design has been changed to increase the spacing between the bezel and the toneCarrier to 5mm to accommodate  toneCarrier tray accessories.</small>

#### Valoi 360 Holder/Advancer adapter

![](<images/scanlight_v4_valoi360_adapter_2.svg>)
<small>All dimensions in millimeters. Mounting holes are sized for M4 socket head cap screws. As of 2026-06-20, the design has been updated to support using the Valoi 360 35mm and medium format film holders without the Advancer. The offset mounting slots and removable snap-in mask are for use with the 35mm holder which has an off-center film gate.</small>
...
<small>PCB rev. A shown, rev. B similar</small>

Scanlight v4 integrates a RP2040 32-bit ARM Cortex-M0+ microcontroller, four [TPS61169](https://www.ti.com/lit/ds/symlink/tps61169.pdf?ts=1764654827638&ref_url=https%253A%252F%252Fwww.google.com%252F) constant-current boost converters configured for 90mA output current at up to 38V, and an array of 36 2835 mid-power LEDs on a 107.4x87mm four-layer PCB. Revision B (26a901**b** marked on label) adds electrically isolated photorelay output drivers for both the shutter and focus signals on the 3.5mm jack.

![](images/scanlight_v4b_pcb_cad.png)

All design files for the PCB can be downloaded from the [GitHub repository](https://github.com/jackw01/scanlight/tree/main/pcb/sl_v4b).

[scanlight v4b schematic (PDF)](pcb/sl_v4b/sl_v4b_20260628.pdf)

[scanlight v4b PCB BOM (CSV)](pcb/sl_v4b/sl_v4b_20260628.csv)
